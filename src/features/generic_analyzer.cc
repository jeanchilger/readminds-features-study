...
void GenericAnalyzer::SetLandmarks(
        mediapipe::NormalizedLandmarkList landmarks) {
...
void GenericAnalyzer::Initialize(
        mediapipe::NormalizedLandmarkList landmarks,
        int img_width, int img_height) {
    img_width_ = img_width;
    img_height_ = img_height;
...
void GenericAnalyzer::CalculateNormFactor() {
    ...
    norm_factor_ = EuclideanDistance(
            cv::Point(
                    static_cast<int>(floor(first_anchor.x() * img_width_)),
                    static_cast<int>(floor(first_anchor.y() * img_height_))),
            cv::Point(
                    static_cast<int>(floor(last_anchor.x() * img_width_)),
                    static_cast<int>(floor(last_anchor.y() * img_height_))));
}

cv::Point GenericAnalyzer::CvtNormIntoCvPoint(
        mediapipe::NormalizedLandmark landmark) {
    int x = static_cast<int>(floor(landmark.x() * img_width_));
    int y = static_cast<int>(floor(landmark.y() * img_height_));
